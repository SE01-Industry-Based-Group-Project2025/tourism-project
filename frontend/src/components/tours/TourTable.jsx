...
export default function TourTable({ tours, onView, onEdit, onDelete, onCreateFromTemplate, isTemplateMode = false }) {
    return (        <div className="bg-white/90 backdrop-blur-md rounded-2xl shadow-xl overflow-hidden border border-white/20">
            <Table>
                    ...
                    <TableRow className="border-b border-gray-100/50">
                        <TableHead className="text-xs font-bold text-gray-700 uppercase tracking-wider">
                            <div className="flex items-center gap-3">
                                ...
                                <span>{isTemplateMode ? 'Template Name' : 'Tour Name'}</span>
                            </div>
                        </TableHead>
                        {!isTemplateMode && (
                          <TableHead className="text-xs font-bold text-gray-700 uppercase tracking-wider">
                ...
                <TableBody className="bg-white/50 backdrop-blur-sm divide-y divide-gray-100/50">
                    {tours.map((tour) => (
                        <TableRow key={tour.id} className="hover:bg-gradient-to-r hover:from-blue-50/40 hover:to-purple-50/20 transition-all duration-300 group hover:shadow-md">
                           ...
                           {!isTemplateMode && <TableCell className="text-gray-700 font-medium">{tour.duration}</TableCell>}
                           {!isTemplateMode && <TableCell className="text-gray-700 font-medium">{tour.participants} spots</TableCell>}
                           {!isTemplateMode && <TableCell className="text-green-600 font-bold">{tour.price}</TableCell>}
                           {!isTemplateMode && (
                            <TableCell>
                                <span
                                    ...
                                    {isTemplateMode ? (
                                        <>
                                            <Button
                                                ...
                                                onClick={() => onCreateFromTemplate && onCreateFromTemplate(tour.id)}
                                            >
                                                <Plus className="w-4 h-4 mr-2" />
                                                Create Tour
                                                <span className="sr-only">Create Tour from Template</span>
                                            </Button>
                                            <Button
                                                variant="ghost"
                                                size="sm"

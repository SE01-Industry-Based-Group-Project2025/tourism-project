  ...
  const fetchCustomTourRequests = async () => {
    ...
    try {
      ...
      const transformedRequests = data.map(request => ({
        id: `QR-${request.id}`,
        customer: request.name || request.customerName || 'Unknown Customer',
        email: request.email || request.user?.email || request.createdBy?.email || 'No email provided',
        tour: request.tourName || request.name || 'Custom Tour Request',
        travelers: request.groupSize || request.group_size || request.numberOfTravelers || 1,
        budget: request.price || request.budget ? `$${request.price || request.budget}` : 'Budget not specified',
        dates: (() => {
          // Handle different date field combinations
          const startDate = request.startDate || request.preferredStartDate || request.start_date || request.travel_start_date;
          const endDate = request.endDate || request.preferredEndDate || request.end_date || request.travel_end_date;
        ...
        submitted: request.createdAt || request.created_at
          ...
          : 'Unknown',
        notes: request.specialRequirements || request.special_requirements || request.requirements || request.description || 'No special requirements',
        ...
        region: request.region || request.travelRegion || 'Region not specified',
        activities: request.activities || request.selectedActivities || [],
        // Additional fields for better display
        rawStartDate: request.startDate || request.preferredStartDate || request.start_date || request.travel_start_date,
        rawEndDate: request.endDate || request.preferredEndDate || request.end_date || request.travel_end_date
      }));
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            <div key={request.id} className="bg-gray-50 rounded-lg p-6 hover:bg-gray-100 transition-colors">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <a href={`mailto:${request.email}`} className="text-sm text-blue-600 hover:text-blue-800 hover:underline">
                      {request.email}
                    </a>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                ...
                <div className="bg-white p-3 rounded-lg">
                  ...
                  <div className="font-medium text-gray-900">
                    {request.rawStartDate && request.rawEndDate ? (
                      <div className="space-y-1">
                        <div className="text-sm">
                          <span className="text-green-600 font-medium">Start:</span> {new Date(request.rawStartDate).toLocaleDateString('en-US', { 
                            weekday: 'short',
                            year: 'numeric', 
                            month: 'short', 
                            day: 'numeric' 
                        ...
                        <div className="text-sm">
                          <span className="text-red-600 font-medium">End:</span> {new Date(request.rawEndDate).toLocaleDateString('en-US', { 
                            weekday: 'short',
                            year: 'numeric', 
                            month: 'short', 
                            day: 'numeric' 
                        ...
                        <div className="text-xs text-gray-500 mt-1">
                          ({Math.ceil((new Date(request.rawEndDate) - new Date(request.rawStartDate)) / (1000 * 60 * 60 * 24))} days)
                        </div>
                      </div>
                    ) : request.rawStartDate ? (
                      <div>
                        <span className="text-green-600 font-medium">From:</span> {new Date(request.rawStartDate).toLocaleDateString('en-US', { 
                          weekday: 'short',
                          year: 'numeric', 
                          month: 'short', 
                          day: 'numeric' 
                      ...
                      <div>
                        <span className="text-red-600 font-medium">Until:</span> {new Date(request.rawEndDate).toLocaleDateString('en-US', { 
                          weekday: 'short',
                          year: 'numeric', 
                          month: 'short', 
                          day: 'numeric' 
                      ...
                      <span className="text-gray-500 italic">Dates not specified</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              {request.notes && request.notes !== 'No special requirements' && (
